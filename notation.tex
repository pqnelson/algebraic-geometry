...
\let\Intersection\Intersects % synonym

\let\propersubset\subset
\let\og@subset\subset
\let\og@supset\supset
...
\let\subgroup\leq
\Def\TrivialSubgroup#1{\mathbf{1}_{#1}}
\Def\Cl{\mathop{\rm Cl}\nolimits} % conjugacy class
\Def\normalizer#1#2{\mathop{\rm N}\nolimits_{#1}(#2)}
\Def\normalsubgroup{\mathrel{\trianglelefteq}}
\Def\propernormalsubgroup{\mathrel{\triangleleft}}
%\Def\subnormalsubgroup{\mathrel{\joinrel{\normalsubgroup\normalsubgroup}}}
\Def\subnormalsubgroup{\mathrel{\trianglelefteq\mkern-2mu\trianglelefteq}}
\Def\subgroups{\mathop{\rm SubGr}\nolimits}
\let\SubGr\subgroups
\Def\join{\sqcup}


\Def\gr#1{\langle#1\rangle}
\Def\FrattiniSubgroup{\mathop{\Phi}\nolimits}

\let\iso\cong
\Def\nathom#1{q_{#1}} % natural morphism to quotient group
%\Def\Index#1#2{\hbox{\texttt{[}}#1:#2\hbox{\texttt{]}}} % \Index{group}{subgroup}
\Def\Index#1#2{[#1:#2]}
\Def\LeftCosets#1#2{#2/#1} % \LeftCosets{subgroup}{group}
\Def\RightCosets#1#2{#1\backslash#2} % \RightCosets{subgroup}{group}
\Def\DoubleCosets#1#2#3{#1\backslash#2/#3} % \DoubleCosets{subgroup}{group}{subgroup}
\Def\CyclicGroup#1{C_{#1}} % cyclic group
\Def\SylowSubgroups{\mathop{\rm Syl}\nolimits}
\let\Syl\SylowSubgroups
...
\Def\Fun{\mathop{\rm Fun}\nolimits} % functor category, analogous to \hom
...
%% Ring Theory
\Def\ndivides{\mathrel{\mskip-\thickmuskip\mskip0.5\thinmuskip{\not\mskip-1mu\big|}\mskip0.5\thinmuskip}}%\mskip-\thickmuskip}}
\Def\divides{\mathrel{\mskip-\thickmuskip\mskip0.5\thinmuskip\big|\mskip0.5\thinmuskip\mskip-\thickmuskip}}
...
\Def\JacobsonRadical{\mathop{\rm rad}\nolimits}
