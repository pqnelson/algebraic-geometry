...
set of a structure which we refer to by the same identifier $S$, or
sometimes referencing a different field $G\smt\structure{U,\dots,G,\dots}$.
\end{remark}
...
We call a magma $M$ \define{Commutative}\mml{group_1:def~12} if for all $x,y\in M$ we have
$x\cdot y=y\cdot x$. In this case, it is common to use $+$ instead of
$\cdot$ for the binary operator. 
\end{definition}

\begin{remark}
When we have a commutative magma $M$, we need to be careful to
distinguish it from an additive magma (which underly a module over a
ring, and many other structures). The Working Mathematician makes no
such distinction.
\end{remark}
