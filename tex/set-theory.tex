...
\begin{definition}[Subset and proper subset predicates]\mml{tarski:def 3}
Let $X$ and $Y$ be sets. We define the predicate $X\subset Y$ to mean:
for all objects $x$ we have $x\in X$ implies $x\in Y$.

We\mml{xboole_0:def 8}\ define the predicate $X\propersubset Y$ (and say ``$X$ is a
...
\begin{definition}[Cartesian product]\mml{zfmisc_1:def 2}
...
Let\mml{zfmisc_1:def 3}\ $X_{3}$ be another set. We define the Cartesian product
$X_{1}\times X_{2}\times X_{3}$ to be equal to $(X_{1}\times X_{2})\times X_{3}$.

Let\mml{zfmisc_1:def 4}\ $X_{4}$ be another set. We define the Cartesian product
$X_{1}\times X_{2}\times X_{3}\times X_{4}$ to be equal to
$(X_{1}\times X_{2}\times X_{3})\times X_{4}$.
\end{definition}

\begin{theorem}[Cartesian product of empty set is empty]\mml{finseqop:2}
...
\begin{definition}[Set difference]\mml{xboole_0:def 5}\label{xboole-0:def-5}
Let $X$ and $Y$ be sets.
...
\begin{definition}[Element of a set]\mml{subset_1:def 1}
Let $X$ be a set.
We define the [soft type] \define{Element} of $X$ to be a set $x$ such
that $x\in X$ if $X$ is nonempty.

(If $X$ is empty, then for the sake of well-definedness, we could take $x=\emptyset$.)
\end{definition}

\begin{remark}[``Element'' is not a well-defined notion]
...
\begin{definition}[Subset of a set]\mml{subset_1}
Let $E$ be a set.
We define the [soft type] \define{Subset} of $E$ to be an element of
its powerset $\powerset{E}$. 
\end{definition}

\begin{definition}[Complement of a subset]\mml{subset_1:def 4}
Let $E$ be a set. Let $A$ be a subset of $E$.
We define the new term the \define{Complement} of $A$ to be the subset
$\Complement{A}$ of $E$ equal to the set difference $E\setminus A$.
\end{definition}

\begin{theorem}[Empty set is always a subset of any set]\mml{subset_1:1}\label{subset_1:1}%
Let $E$ be a set. Then $\emptyset$ is a subset of $E$.
\end{theorem}

\begin{definition}[Intersection of a family of sets]\mml{setfam_1:def 1}\label{setfam-1:def-1}
Let $\mathcal{F}$ be a set. We define the \define{Intersection} of $\mathcal{F}$
...
\begin{theorem}[Intersection of a family is always contained in any family member]\mml{setfam_1:3}
...
\begin{theorem}[Intersection of a family containing an empty set is empty]\mml{setfam_1:4}\label{setfam_1:4}%
...
\begin{theorem}[Complement of intersection is union of member-wise complements]\mml{setfam_1:27}\label{setfam_1:27}
...
\begin{definition}[$X$-defined and $X$-valued relations]
Let $X$ be a set, let $R$ be a relation.

We call $R$ \mml{relat_1:def 18}\define{$X$-defined} when $\dom(R)\subset X$.

We call $R$ \mml{relat_1:def 19}\define{$X$-valued} when $\rng(R)\subset X$.
\end{definition}
...
\begin{theorem}[Empty set is initial object]\mml{PARTFUN1:73,74}
...
\begin{theorem}[For any set, there is a proper class of Grothendieck universes containing it]
Let $X$ be a set. Then there is no set $G$ such that for all sets
$U$ we have $U\in G$ iff $U$ is a Grothendieck universe and $X\in u$.
\end{theorem}

\begin{proof}
We can construct a sequence $G_{\alpha}$ indexed by ordinals such that
$G_{0}=\GrothendieckUniverse{X}$, and for all successor ordinals
$\succ{A}$ we have
$G_{\succ{A}}=\GrothendieckUniverse{\powerset{G_{A}}}$, and for any
limit ordinal $\lambda$ we have $G_{\lambda}=\GrothendieckUniverse{\powerset{\Union_{\alpha<\lambda}G_{\alpha}}}$.
This is clearly an increasing sequence: for any ordinals $A<B$ we have
$G_{A}\propersubset G_{B}$. This gives us an injection from the proper
class of all ordinals to a (necessarily proper) class of Grothendieck universes
$\{G_{A}\mid A\mbox{ is an ordinal}\}$ each of which contains $X\in G_{A}$.
This may not even be \emph{all} possible Grothendieck universes
containing $X$, and we \emph{already} have a proper class.
\end{proof}
...
\begin{definition}[$U$-sets]\mml{classes5:def 8}
Let $U$ be a nonempty set, let $x$ be an object. We define an
attribute, calling $x$ an \define{$U$-set} if $x\in U$.

We can equally define a new type, \mml{classes5:def 9}\define{Set of $U$} which is a set $Y$
such that $Y\in U$ (i.e., $Y$ is an $U$-set).
\end{definition}

\begin{definition}[$U$-classes]\mml{classes5:def 10}
Let $U$ be a universe, let $x$ be an object. We define an attribute,
saying $x$ is \define{$U$-Class} if $x\in\powerset{U}$ and $x\notin U$.

We could equally define a new type, \mml{classes5:def 11}\define{class of $U$}, which
describes a $U$-Class.
\end{definition}
